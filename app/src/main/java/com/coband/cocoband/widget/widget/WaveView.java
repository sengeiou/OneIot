...
import com.coband.common.tools.LinearEvaluator;
...
/**
 * Created by mai on 17-1-19.
 */

public class WaveView extends View implements LinearEvaluator.EvaluatorListener, Animator.AnimatorListener{


    private final long DURATION_DRAW_WAVE = 330;

    private final int WAVE_PEAK_LEFT_HEIGHT = 1;

    /**The max peek(contains the wave line) that will be shown*/
    private final int MAX_VALID_ENTRIES = 10;

    /**the view width as 9 weights*/
    private final int WIDTH_TOTAL_WEIGHTS = 9;
    /**the view height as 9 weights*/
    private final int HEIGHT_TOTAL_WEIGHTS = 9;
    ...
    private final int[][] m_WavePeak_Weight = new int[][]{
            new int[]{4, 0, 6, 6},
    ...
    //9
    //10
    private final int[] m_WaveLine_Weight = new int[]{WIDTH_TOTAL_WEIGHTS, 0, 6, 6};
    ...
    private void setValues(){
        ...
        m_PeakPaint.setColor(Color.parseColor("#FF6A6D"));
        m_PeakPaint.setAntiAlias(true);
        m_PeakPaint.setStyle(Paint.Style.STROKE);
...
//            	emulate test
                ...
                if(getVisibility() != View.VISIBLE){
                    Logger.e("", "The view is hidden by others");
                }
    ...
    private class WaveViewController{
