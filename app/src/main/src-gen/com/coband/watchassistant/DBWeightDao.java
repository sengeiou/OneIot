package com.coband.watchassistant;

import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteStatement;

import de.greenrobot.dao.AbstractDao;
import de.greenrobot.dao.Property;
import de.greenrobot.dao.internal.DaoConfig;

import com.coband.watchassistant.DBWeight;

// THIS CODE IS GENERATED BY greenDAO, DO NOT EDIT.
/** 
 * DAO for table "DBWEIGHT".
*/
public class DBWeightDao extends AbstractDao<DBWeight, Long> {

    public static final String TABLENAME = "DBWEIGHT";

    /**
     * Properties of entity DBWeight.<br/>
     * Can be used for QueryBuilder and for referencing column names.
    */
    public static class Properties {
        public final static Property Id = new Property(0, Long.class, "id", true, "_id");
        public final static Property Weight = new Property(1, float.class, "weight", false, "WEIGHT");
        public final static Property BodyMI = new Property(2, float.class, "bodyMI", false, "BODY_MI");
        public final static Property Upload = new Property(3, boolean.class, "upload", false, "UPLOAD");
        public final static Property Date = new Property(4, long.class, "date", false, "DATE");
        public final static Property Uid = new Property(5, String.class, "uid", false, "UID");
    };


    public DBWeightDao(DaoConfig config) {
        super(config);
    }
    
    public DBWeightDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    /** Creates the underlying database table. */
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        String constraint = ifNotExists? "IF NOT EXISTS ": "";
        db.execSQL("CREATE TABLE " + constraint + "\"DBWEIGHT\" (" + //
                "\"_id\" INTEGER PRIMARY KEY ," + // 0: id
                "\"WEIGHT\" REAL NOT NULL ," + // 1: weight
                "\"BODY_MI\" REAL NOT NULL ," + // 2: bodyMI
                "\"UPLOAD\" INTEGER NOT NULL ," + // 3: upload
                "\"DATE\" INTEGER NOT NULL UNIQUE ," + // 4: date
                "\"UID\" TEXT NOT NULL );"); // 5: uid
    }

    /** Drops the underlying database table. */
    public static void dropTable(SQLiteDatabase db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"DBWEIGHT\"";
        db.execSQL(sql);
    }

    /** @inheritdoc */
    @Override
    protected void bindValues(SQLiteStatement stmt, DBWeight entity) {
        stmt.clearBindings();
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(1, id);
        }
        stmt.bindDouble(2, entity.getWeight());
        stmt.bindDouble(3, entity.getBodyMI());
        stmt.bindLong(4, entity.getUpload() ? 1L: 0L);
        stmt.bindLong(5, entity.getDate());
        stmt.bindString(6, entity.getUid());
    }

    /** @inheritdoc */
    @Override
    public Long readKey(Cursor cursor, int offset) {
        return cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0);
    }    

    /** @inheritdoc */
    @Override
    public DBWeight readEntity(Cursor cursor, int offset) {
        DBWeight entity = new DBWeight( //
            cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0), // id
            cursor.getFloat(offset + 1), // weight
            cursor.getFloat(offset + 2), // bodyMI
            cursor.getShort(offset + 3) != 0, // upload
            cursor.getLong(offset + 4), // date
            cursor.getString(offset + 5) // uid
        );
        return entity;
    }
     
    /** @inheritdoc */
    @Override
    public void readEntity(Cursor cursor, DBWeight entity, int offset) {
        entity.setId(cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0));
        entity.setWeight(cursor.getFloat(offset + 1));
        entity.setBodyMI(cursor.getFloat(offset + 2));
        entity.setUpload(cursor.getShort(offset + 3) != 0);
        entity.setDate(cursor.getLong(offset + 4));
        entity.setUid(cursor.getString(offset + 5));
     }
    
    /** @inheritdoc */
    @Override
    protected Long updateKeyAfterInsert(DBWeight entity, long rowId) {
        entity.setId(rowId);
        return rowId;
    }
    
    /** @inheritdoc */
    @Override
    public Long getKey(DBWeight entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
        }
    }

    /** @inheritdoc */
    @Override    
    protected boolean isEntityUpdateable() {
        return true;
    }
    
}
