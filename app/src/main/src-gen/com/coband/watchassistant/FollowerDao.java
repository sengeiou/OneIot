...
public class FollowerDao extends AbstractDao<Follower, String> {
    ...
    */
    ...
    private Query<Follower> user_FollowersQuery;
    ...
    /** @inheritdoc */
    @Override
    protected String updateKeyAfterInsert(Follower entity, long rowId) {
        return entity.getObjectId();
    }
    
    /** @inheritdoc */
    @Override
    public String getKey(Follower entity) {
        if(entity != null) {
            return entity.getObjectId();
        } else {
            return null;
    ...
    /** Internal query to resolve the "followers" to-many relationship of User. */
    public List<Follower> _queryUser_Followers(Long userId) {
        synchronized (this) {
            if (user_FollowersQuery == null) {
                QueryBuilder<Follower> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.UserId.eq(null));
                user_FollowersQuery = queryBuilder.build();
            }
        }
        Query<Follower> query = user_FollowersQuery.forCurrentThread();
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(" FROM FOLLOWER T");
    ...
    protected Follower loadCurrentDeep(Cursor cursor, boolean lock) {
        Follower entity = loadCurrent(cursor, 0, lock);
    ...
    public Follower loadDeep(Long key) {
        assertSinglePk();
        if (key == null) {
            return null;
        }
    ...
    public List<Follower> loadAllDeepFromCursor(Cursor cursor) {
        int count = cursor.getCount();
        List<Follower> list = new ArrayList<Follower>(count);
    ...
    protected List<Follower> loadDeepAllAndCloseCursor(Cursor cursor) {
        try {
    ...
    public List<Follower> queryDeep(String where, String... selectionArg) {
