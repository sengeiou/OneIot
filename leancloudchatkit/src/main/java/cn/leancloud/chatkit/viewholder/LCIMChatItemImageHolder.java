...
public class LCIMChatItemImageHolder extends LCIMChatItemHolder {
    ...
    public void initView() {
        ...
        if (isLeft) {
            contentView.setBackgroundResource(R.drawable.message_chat_white_bg);
        } else {
            contentView.setBackgroundResource(R.drawable.message_chat_gray_bg);
        }

        contentView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                EventBus.getDefault().post(new ImageViewClickEvent(((AVIMImageMessage) message).getFileUrl(), contentView));
//                try {
...
//                }
    ...
    public void bindData(Object o) {
        ...
        if (message instanceof AVIMImageMessage) {
            ...
            double actualHight = 0;
            double actualWidth = 0;
            try {
                // 图片的真实高度与宽度

                actualHight = imageMsg.getHeight();
                actualWidth = imageMsg.getWidth();
            } catch (Exception e) {
            }
