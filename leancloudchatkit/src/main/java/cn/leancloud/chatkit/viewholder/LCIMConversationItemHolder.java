...
import cn.leancloud.chatkit.utils.DateUtils;
...
public class LCIMConversationItemHolder extends LCIMCommonViewHolder {
  ...
  public void initView() {
    ...
    ivMute = (ImageView) itemView.findViewById(R.id.iv_mute);
    ...
    if (null != conversation) {
      ...
      itemView.setOnLongClickListener(new View.OnLongClickListener() {
        ...
        public boolean onLongClick(View v) {
          ...
          builder.setItems(new String[]{getContext().getString(R.string.delete_this_chat)}, new DialogInterface.OnClickListener() {
      ...
      ArrayList<String> muteList = SpUtils.getMuteList();
      if (muteList.contains(conversation.getConversationId())) {
  ...
  private void updateIcon(AVIMConversation conversation) {
    if (null != conversation) {
      if (conversation.isTransient() || conversation.getMembers().size() > 2) {
        avatarView.setImageResource(R.drawable.profile_men);
      } else {
        LCIMConversationUtils.getConversationPeerIcon(conversation, new AVCallback<String>() {
          ...
          protected void internalDone0(String s, AVException e) {
            ...
            if (!TextUtils.isEmpty(s)) {
              Glide.with(getContext()).load(s).centerCrop().fitCenter()
                .error(R.drawable.profile_men).into(avatarView);
            } else {
              avatarView.setImageResource(R.drawable.profile_men);
            }
          }
  ...
  private void updateLastMessage(AVIMMessage message) {
    if (null != message) {
      ...
      timeView.setText(DateUtils.getMessageTime(date.getTime(), getContext()));
...
//      getContext().startActivity(intent);
      EventBus.getDefault().post(new IMCOChatEvent(conversation.getConversationId()));
